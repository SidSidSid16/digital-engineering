...
signal INT1_pipe3 : UNSIGNED (data_size+1 downto 0);
signal INT2_pipe3 : UNSIGNED (data_size*2-1 downto 0);
signal INTD_pipe3 : UNSIGNED (data_size-1 downto 0);
signal INT5_pipe3 : UNSIGNED (data_size downto 0);
...
begin
  if rising_edge(clk) then
    if rst = '1' then 
      ...
      INTD_pipe1 <= unsigned(INTD_pipe3);
      INT5_pipe1 <= unsigned(INT5_pipe3);
    end if;
  end if;
...
begin
  if rising_edge(clk) then
    if rst = '1' then 
      INT1_pipe3 <= (others => '0');
      INT2_pipe3 <= (others => '0');
      INTD_pipe3 <= to_unsigned(1,INTD'length); -- type conversion notation
      INT5_pipe3 <= (others => '0');
    else
      INT1_pipe3 <= unsigned(INT1);
      INT2_pipe3 <= unsigned(INT2);
      INTD_pipe3 <= unsigned(INTD);
      INT5_pipe3 <= unsigned(INT5);
    end if;
  end if;
