...
entity TOP_LEVEL is
  ...
  Port ( 
    GCLK : in  STD_LOGIC; -- 100MHz clock
    -- Button inputs (4=U, 3=C, 2=R, 1=L, 0=D)
    --   BTN(0) - BTND - is wired to global reset			  
...
signal CLK_26MHZ, CLK_100MHZ : STD_LOGIC;

-- Debounced button inputs (4=U, 3=C, 2=R, 1=L, 0=D)
signal btn_db_26MHz, btn_db_100MHz : STD_LOGIC_VECTOR (4 downto 0); 
...
debouncers: for i in 0 to 4 generate
	...
	PORT MAP(
		CLK => CLK_26MHZ,
		Sig => BTN(i),
		Deb_Sig => btn_db_26MHZ(i) -- (4=U, 3=C, 2=R, 1=L, 0=D)
	);
...
PORT MAP(
    CLK => CLK_100MHZ,
    RST => btn_db_100MHZ(0), -- Note: PB(0) reserced for reset (BTND) 
    USER_PB => btn_db_100MHZ,
    SWITCHES => SW,
    LEDS => LD_INT,
...
PORT MAP (
  src_clk => CLK_100MHZ,    -- used only when SRC_INPUT_REG = 1
  src_in => en_spi, 
  dest_clk => CLK_26MHZ,
